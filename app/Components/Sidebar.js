...
class Sidebar extends Component {
  ...
  render() {
    ...
    let reminders = this.state.reminders.map(reminder => {
      ID++
      return <Reminder key={ID} reminder={reminder} />
    })

    if (this.state.reminders.length === 0) {
      reminders = (<p>No reminders</p>)
    }
      ...
      <div className='uk-offcanvas-bar'>
        ...
        <table className="uk-table uk-table-hover uk-table-middle uk-table-divider uk-table-small">
          ...
          <tbody style={{fontSize: '12px'}} id='remindersTable'>
            <tr>
              <td style={{paddingLeft: '0px'}}><input className="uk-input uk-form-blank" type="text" placeholder="Add reminder" autoFocus/></td>
